...
class YoloDetector(Node):
    ...
    def detect(self, mode):
        ...
        target_ids = [k for k,v in yolo.names.items() if v == mode]
        results = yolo(source=im, device="0", stream=False, verbose=False, conf=0.5, classes=[target_ids])
    ...
    def analyze_results(self, results, image, mode):
        ...
        for result in results:
            ...
            for box in boxes:
                if mode == 'stop':
                    xyxy = box.xyxy
                    sign_image = image[xyxy[0]:xyxy[2], xyxy[1]:xyxy[3]]
